...
class Note {
  ...
  add (note) {
    ...
    const mongo = async (newNote) => {
      const note = new TheNotes(newNote);
      await note.save();
      mongoose.disconnect();
    };
    let newNote = {
  ...
  async list(category){
    if(typeof(category)=='string'){
      const record = await TheNotes.find({category:category});
      record.forEach(el=>{
        console.log(`${el.text}
      ...
      const notes = await TheNotes.find({});
      notes.forEach(el=>{
        console.log(`${el.text}
        Category: ${el.category}    ID: ${el.id}
        ----------------------------------------------------`);
  ...
  async delete(id){
    await TheNotes.findByIdAndDelete({_id: id});
